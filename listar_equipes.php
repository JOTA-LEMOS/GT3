...
<div class="container py-4">
  ...
  <form method="GET" class="row g-5 mb-4 justify-content-center">
    <div class="col-md-3">
  ...
  <?php if ($result->num_rows > 0): ?>
    <?php while ($row = $result->fetch_assoc()): ?>
      <?php
        ...
        // Logo do campeonato (BLOB → Base64)
        $logo_base64 = '';
        $logo_binario = $row['campeonato_logo'];
        if (!empty($logo_binario)) {
          $logo_base64 = 'data:image/png;base64,' . base64_encode($logo_binario);
        } else {
          $logo_base64 = 'https://via.placeholder.com/100x100?text=?';
        }

        // Foto do carro (BLOB → Base64)
        $foto_base64 = '';
        $foto_binario = $row['foto'];
        if (!empty($foto_binario)) {
            $foto_base64 = 'data:image/jpeg;base64,' . base64_encode($foto_binario);
        } else {
            $foto_base64 = 'https://via.placeholder.com/360x200?text=Sem+Imagem';
        }
      ?>
      ...
      <div class="card-gt3">
        ...
        <img src="<?= $foto_base64 ?>" class="car-image" alt="<?= htmlspecialchars($row['modelo']) ?>">
        ...
        <div class="championship-logo">
          <img src="<?= $logo_base64 ?>" alt="Logo do Campeonato" width="60" height="60" />
        </div>
      </div>
    <?php endwhile; ?>
  <?php else: ?>
